...
class Offtaker extends Admin_Controller
{
	...
	{
		$this->title 	= "Offtaker";
		$this->content 	= "offtaker/index";
		$this->assets 	= array('assets');
		$wilayah = $this->m_perusahaan->all_kategori(['offtaker','multi'])->result();
		$data['perusahaan']['offtaker'] = 'All';
	...
	public function simpan($id="")
	{
		...
		if (empty($id)){
			...
			if ($save){
				...
				redirect('admin/offtaker/edit/'.base64_encode($id));
		...
		foreach ($list as $item) {
			...
			$btngroup = '<div class="input-group">
					...
					<ul class="dropdown-menu">
						<li>' . anchor("admin/offtaker/edit/".base64_encode($item->no),"<i class=\"fa fa-edit\"></i>Edit") . '</li>
						<li>' . anchor("admin/offtaker/hapus/".base64_encode($item->no),"<i class=\"fa fa-trash\"></i>Hapus") . '</li>
					</ul>
                </div>';
